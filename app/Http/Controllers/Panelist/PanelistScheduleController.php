...
class PanelistScheduleController extends Controller
{
    /**
                    ...
                    ->map(function (string $stage, string $phaseKey) use ($conceptTitlesByGroupId, $group, $panelistId, $schedulesByGroupStage): array {
                        ...
                        return [
                            ...
                            'group' => [
                                ...
                                'coPanelists' => $this->resolveCoPanelists($group, $panelistId),
                            ],
                        ];
                    })
                    ->values();
     ...
     * @return array<int, array{id: int|null, name: string|null, role: string, email: string|null}>
     */
    private function resolveCoPanelists(Group $group, ?int $panelistId): array
    {
        return $group->panelAssignments
            ->filter(function (GroupPanelist $assignment) use ($panelistId): bool {
                if ($panelistId === null) {
                    return true;
                }

                return (int) $assignment->panelist_id !== $panelistId;
            })
            ->sortBy('panel_slot')
            ->values()
