...
use App\Http\Requests\Panelist\MarkAllPanelistNotificationsReadRequest;
...
class PanelistNotificationController extends Controller
{
    ...
    public function markAllAsRead(MarkAllPanelistNotificationsReadRequest $request): RedirectResponse
    {
        $userId = $request->user()?->id;
        if ($userId === null) {
            abort(403);
    ...
    private function buildNotifications(int $userId): Collection
    {
        return $this->buildSiteWideNotifications()
            ->concat($this->buildAssignmentNotifications($userId))
        ...
        return $siteWideNotificationModel
            ...
            ->map(function (SiteWideNotification $notification): array {
                ...
                return [
                    ...
                    'action_url' => route('panelist.dashboard'),
     ...
     * @return Collection<int, array<string, mixed>>
     */
    private function buildAssignmentNotifications(int $userId): Collection
    {
        if (! Schema::hasTable('group_panelists') || ! Schema::hasTable('groups')) {
            return collect();
        }

        return GroupPanelist::query()
            ->with(['group:id,name'])
            ->where('panelist_id', $userId)
            ->orderByDesc('updated_at')
            ->orderByDesc('id')
            ->limit(30)
            ->get(['id', 'group_id', 'panel_slot', 'role', 'created_at', 'updated_at'])
            ->map(function (GroupPanelist $assignment): array {
                $groupName = is_string($assignment->group?->name) && trim($assignment->group->name) !== '' ? trim($assignment->group->name) : 'Group';
                $notificationDate = $assignment->updated_at ?? $assignment->created_at;
                $panelRole = is_string($assignment->role) && trim($assignment->role) !== '' ? trim($assignment->role) : 'Panelist';
                $slotLabel = is_numeric($assignment->panel_slot) ? 'Slot '.(int) $assignment->panel_slot : null;
                $meta = collect([$panelRole, $slotLabel])->filter()->implode(' • ');

                return [
                    'key' => 'panelist-assignment-'.$assignment->id,
                    'type' => 'Assignment',
                    'title' => 'Group assignment updated',
                    'message' => 'You are assigned to '.$groupName.'.',
                    'meta' => $meta !== '' ? $meta : null,
                    ...
                    'action_url' => route('panelist.assigned-groups'),
    ...
    private function buildSubmissionNotifications(int $userId): Collection
    {
        ...
        return DocumentSubmission::query()
            ...
            ->whereHas('group.panelAssignments', fn ($query) => $query->where('panelist_id', $userId))
            ...
            ->map(function (DocumentSubmission $submission): array {
                ...
                return [
                    'key' => 'panelist-submission-'.$submission->id,
                    'type' => 'Document',
                    ...
                    'created_at' => $createdAt?->toIso8601String(),
                    'created_at_label' => $createdAt?->format('Y-m-d H:i') ?? 'Unknown time',
                    'action_url' => route('panelist.documents'),
                    'timestamp' => $createdAt?->getTimestamp() ?? 0,
                ];
            })
            ->values();
    ...
    private function buildScheduleNotifications(int $userId): Collection
    {
        ...
        return DefenseSchedule::query()
            ...
            ->map(function (DefenseSchedule $schedule): array {
                ...
                return [
                    'key' => 'panelist-schedule-'.$schedule->id,
                    'type' => 'Schedule',
                    ...
                    'created_at' => $notificationDate?->toIso8601String(),
                    'created_at_label' => $notificationDate?->format('Y-m-d H:i') ?? 'Unknown time',
                    'action_url' => route('panelist.schedule'),
                    'timestamp' => $notificationDate?->getTimestamp() ?? 0,
                ];
            })
            ->values();
