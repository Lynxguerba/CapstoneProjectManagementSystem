...
use App\Http\Requests\Instructor\MarkAllInstructorNotificationsReadRequest;
...
class InstructorNotificationController extends Controller
{
    ...
    public function markAllAsRead(MarkAllInstructorNotificationsReadRequest $request): RedirectResponse
    {
        $userId = $request->user()?->id;
        if ($userId === null) {
            abort(403);
    ...
    private function buildSubmissionNotifications(int $userId): Collection
    {
        ...
        return DocumentSubmission::query()
            ...
            ->whereHas('group.programSet', fn ($query) => $query->where('instructor_id', $userId))
            ...
            ->map(function (DocumentSubmission $submission): array {
                ...
                return [
                    ...
                    'created_at' => $createdAt?->toIso8601String(),
                    'created_at_label' => $createdAt?->format('Y-m-d H:i') ?? 'Unknown time',
                    'action_url' => route('instructor.requirements.documents.review'),
                    'timestamp' => $createdAt?->getTimestamp() ?? 0,
                ];
            })
            ->values();
        ...
        return DefenseSchedule::query()
            ...
            ->map(function (DefenseSchedule $schedule): array {
                ...
                return [
                    ...
                    'action_url' => route('instructor.scheduling.manage', ['schedule' => $schedule->id]),
