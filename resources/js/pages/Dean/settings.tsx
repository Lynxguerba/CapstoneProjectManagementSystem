            ...
            <div className="space-y-6">
                <div className="max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4">
                        ...
                        <input value={name} onChange={(e) => setName(e.target.value)} className="rounded-xl border px-4 py-2" />

                        <label className="text-sm text-slate-700">Email</label>
                        <input value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-xl border px-4 py-2" />

                        <div className="pt-2">
                            <button onClick={() => alert('UI only: save profile')} className="rounded-xl bg-slate-900 px-4 py-2 text-white">
                                Save
                            </button>
                        </div>
                    </div>
