...
type ProgramChairAuthUser = {
    role?: string;
    roles?: string[];
};

type ProgramChairLayoutPageProps = {
    academicYears?: AcademicYearOption[];
    auth?: {
        user?: ProgramChairAuthUser;
    };
};
...
const ProgramChairpersonLayout = ({ title, subtitle, children }: Props) => {
    const { academicYears = [], auth } = usePage<ProgramChairLayoutPageProps>().props;
    ...
    React.useEffect(() => {
        ...
        const fromQuery = queryAcademicYear ? academicYears.find((year) => year.label === queryAcademicYear) : undefined;
    ...
    const applyAcademicYearFilter = React.useCallback((academicYearLabel: string) => {
        if (typeof window === 'undefined') {
            return;
        }

        const searchParams = new URLSearchParams(window.location.search);
        searchParams.set('academic_year', academicYearLabel);
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            if (selectedAcademicYear?.id !== year.id) {
                                                                applyAcademicYearFilter(year.label);
                                                            }
...
export default ProgramChairpersonLayout;
