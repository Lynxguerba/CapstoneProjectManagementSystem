...
const AdviserConcepts = () => {
    ...
    const selectedConcept = useMemo(
        () => selectedGroup?.concepts.find((c) => c.id === selectedConceptId) ?? null,
                                ...
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="min-w-0">
                                            <div className="truncate text-sm font-semibold text-slate-900">{b.groupName}</div>
                                            <div className="mt-1 text-xs text-slate-500">Updated: {b.updatedAt}</div>
                                        </div>
                                        <span className="inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold whitespace-nowrap text-slate-700">
                                ...
                                {selectedGroup.concepts.map((c) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`rounded-2xl border p-5 text-left transition-colors ${active ? 'border-indigo-200 bg-indigo-50' : 'border-slate-200 bg-white hover:bg-slate-50'}`}
                                        >
                                            <div className="line-clamp-2 text-sm font-semibold text-slate-900">{c.title}</div>
                                            <div className="mt-3 flex items-center justify-between gap-3">
                                                <span
                                                    className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${decisionPill(c.decision)}`}
                                                >
                                                    {c.decision}
                                                </span>
                            ...
                            {!selectedConcept ? (
                                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
                                    ...
                                    <div className="mt-1 text-sm text-slate-600">
                                        Pick one of the three proposals to approve/reject or request revision.
                                    </div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                    <div className="rounded-2xl border border-slate-200 bg-white p-5">
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-2">
                                            <span
                                                className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${decisionPill(selectedConcept.decision)}`}
                                            >
                                    ...
                                    <div className="rounded-2xl border border-slate-200 bg-white p-5">
                                        ...
                                        <div className="mt-4 rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
                                            ...
                                            <div className="mt-1 text-sm text-slate-600">
                                                Only one concept can be marked as approved per group (enforced later in backend).
                                            </div>
                                        </div>
                                    </div>
                                </div>
