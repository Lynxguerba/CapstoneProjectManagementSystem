...
type ScheduleRow = {
    ...
    evaluationStatus: 'Pending' | 'Defended' | 'Conditional' | 'Failed' | string;
    group: ScheduleGroupDetails;
};

type AdviserScheduleProps = {
    rows?: ScheduleRow[];
};
...
const AdviserSchedule = () => {
    const { props } = usePage<AdviserScheduleProps>();
    ...
    const filteredRows = React.useMemo(() => {
        ...
        return activePhaseRows.filter((row) => {
            const searchable = [
                ...
                ...row.group.members.map((member) => member.name ?? ''),
    ...
    return (
        <AdviserLayout title="Defense Schedule" subtitle="Track groups assigned under your guidance by phase">
                    ...
                    <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Groups currently assigned to you for this phase.</p>
                            </div>
                        </div>
                                        ...
                                        <tr key={row.id} className="transition-colors hover:bg-emerald-50/30">
                                            ...
                                            <td className="px-5 py-3.5 text-right">
                                                <div className="inline-flex gap-2">
                                                    ...
                                                    <Link
                                                        href={`/adviser/live-defense?group=${row.group.id}`}
