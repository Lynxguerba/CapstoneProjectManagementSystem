                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-200 text-slate-600">
                                        ...
                                        <th className="py-3 text-left font-semibold">Panel</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {items.map((i) => (
                                        <tr key={i.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="py-3 whitespace-nowrap text-slate-600">{i.date}</td>
                                            <td className="py-3 whitespace-nowrap text-slate-600">{i.time}</td>
                                            <td className="py-3 whitespace-nowrap text-slate-600">{i.room}</td>
                                            <td className="py-3 whitespace-nowrap text-slate-600">{i.panel}</td>
                                            <td className="py-3 whitespace-nowrap">
                                                <span
                                                    className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${statusPill(i.status)}`}
                                                >
                                                    {i.status}
                                                </span>
                                            </td>
                                            <td className="py-3 text-right">
                                                <button
                                                    type="button"
                                                    onClick={() => alert('UI only: view details')}
                    ...
                    >
                        ...
                        <div className="mt-4 grid grid-cols-7 gap-2">
                            {calendarDays.map((d) => {
                                ...
                                return (
                                    <div key={d.key} className="min-h-20 rounded-2xl border border-slate-200 bg-white p-2">
                                        <div className="text-xs font-semibold text-slate-700">{d.label}</div>
                                        <div className="mt-2 space-y-1">
                                            {entries.slice(0, 2).map((e) => (
                                                <div
                                                    key={e.id}
                                                    className="truncate rounded-lg border border-indigo-200 bg-indigo-50 px-2 py-1 text-[10px] font-semibold text-indigo-700"
                                                >
                                                    {e.group}
                                                </div>
                                            ))}
                                            {entries.length > 2 ? (
                                                <div className="text-[10px] font-semibold text-slate-500">+{entries.length - 2} more</div>
                                            ) : null}
                                        </div>
                                    </div>
                                );
