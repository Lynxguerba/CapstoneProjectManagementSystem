...
type AdviserNotificationsProps = {
...
const statusTone = (status?: string | null): string => {
    ...
    if (normalizedStatus === 'approved' || normalizedStatus === 'published' || normalizedStatus === 'scheduled') {
        return 'border-emerald-200 bg-emerald-50 text-emerald-700';
    }
...
const AdviserNotifications = () => {
    const { props } = usePage<AdviserNotificationsProps>();
    ...
    const markAsRead = (notificationKey: string) => {
        mutateNotification(notificationKey, 'patch', `/adviser/notifications/${encodeURIComponent(notificationKey)}/read`);
    };

    const dismissNotification = (notificationKey: string) => {
        mutateNotification(notificationKey, 'delete', `/adviser/notifications/${encodeURIComponent(notificationKey)}`);
    };
    ...
    const markAllAsRead = () => {
        ...
        router.patch(
            '/adviser/notifications/read-all',
            {
    ...
    return (
        <AdviserLayout title="Notifications" subtitle="Track announcements, submissions, and defense schedule updates.">
