...
const AcknowledgementReceiptPage = () => {
    ...
    const authUserId = authUser?.id !== undefined && authUser?.id !== null ? Number(authUser.id) : null;
    const activeRoles = React.useMemo(() => {
        const roles = new Set<string>();
        const primaryRole = typeof authUser?.role === 'string' ? authUser.role.trim().toLowerCase() : '';
        if (primaryRole !== '') {
    ...
    const canOperateESign = activeRoles.has('panelist') || activeRoles.has('adviser');
    const [pendingRowActions, setPendingRowActions] = React.useState<Record<string, 'sign' | 'clear'>>({});
    ...
    const signRow = (facultyRow: FacultyRow): void => {
        if (!group || !facultyRow.userId || selectedDefenseTypeKey === '') {
            return;
        }

        setPendingRowActions((currentRows) => ({
            ...currentRows,
            [facultyRow.id]: 'sign',
        }));

        router.put(
            '/adviser/live-defense/acknowledgement/signature',
            {
                group_id: group.id,
                faculty_user_id: facultyRow.userId,
                defense_type_key: selectedDefenseTypeKey,
            },
            {
                preserveState: true,
                preserveScroll: true,
                only: ['facultyRows', 'flash'],
                onFinish: () => {
                    setPendingRowActions((currentRows) => {
                        const nextRows = { ...currentRows };
                        delete nextRows[facultyRow.id];
    ...
    const clearSignedRow = (facultyRow: FacultyRow): void => {
        if (!group || !facultyRow.userId || selectedDefenseTypeKey === '') {
            return;
        }
        ...
        router.delete('/adviser/live-defense/acknowledgement/signature', {
            data: {
                group_id: group.id,
                faculty_user_id: facultyRow.userId,
                defense_type_key: selectedDefenseTypeKey,
    ...
    return (
        <AdviserLayout title="Acknowledgement Receipt" subtitle="Capstone project defense payment verification">
            <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="space-y-5">
                <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <Link href="/adviser/live-defense" className="font-medium text-slate-600 transition-colors hover:text-slate-900">
                        Live Defense
                    </Link>
                    <ChevronRight className="h-3 w-3 text-slate-400" />
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <Link
                            href={`/adviser/live-defense${groupQuery}`}
                            ...
                            <tbody>
                                {facultyRows.length === 0 ? (
                                    ...
                                    facultyRows.map((facultyRow) => {
                                        ...
                                        const rowAction = pendingRowActions[facultyRow.id] ?? null;
                                        const isRowPending = rowAction !== null;
                                        const canSignThisRow =
                                            canOperateESign &&
                                            authUserId !== null &&
                                            facultyRow.userId === authUserId &&
                                            signatureDataUrl !== null &&
                                            !isRowPending &&
                                            selectedDefenseTypeKey !== '';
                                        const signStateDescription = !canOperateESign
                                            ? 'View only'
                                            : authUserId === null || facultyRow.userId !== authUserId
                                              ? 'Only assigned faculty'
                                              : signatureDataUrl === null
                                                ? 'No e-signature'
                                                : selectedDefenseTypeKey === ''
                                                  ? 'Defense phase unavailable'
                                                  : isRowPending
                                                    ? rowAction === 'sign'
                                                        ? 'Saving signature...'
                                                        : 'Removing signature...'
                                                    ...
                                                    <div className="relative flex min-h-16 items-center justify-center">
                                                        {isSigned && signatureDataUrl ? (
                                                            <>
                                                                <button
                                                                    type="button"
                                                                    onClick={() => clearSignedRow(facultyRow)}
                                                                    disabled={isRowPending}
                                                                    className="absolute top-0.5 right-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full border border-slate-300 bg-white text-[10px] text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-60"
                                                            ...
                                                            <div className="space-y-1">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => signRow(facultyRow)}
                                                                    disabled={!canSignThisRow}
                                                                ...
                                                                >
                                                                    {rowAction === 'sign' ? 'Signing...' : 'Sign'}
                                                                </button>
                                                                {signStateDescription !== '' ? (
                                                                    <p className="text-[10px] text-slate-500">{signStateDescription}</p>
                                                                ) : null}
                                                            </div>
                                                        )}
                                                    </div>
