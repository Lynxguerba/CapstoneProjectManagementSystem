...
type AdviserSelectionPageProps = {
...
const AdviserSelection = () => {
    const { props } = usePage<AdviserSelectionPageProps>();
    const advisers = props.advisers ?? [];
    const academicYears = props.academicYears ?? [];
    const group = props.group ?? null;
    const currentAdviser = props.currentAdviser ?? null;
    const pendingRequest = props.pendingRequest ?? null;
    ...
    const pendingAdviserId = pendingRequest?.adviser_id ?? null;
    const isGroupAssigned = Boolean(currentAdviser?.id);

    const handleInvite = (adviserId: number) => {
        if (processingAdviserId !== null) {
            return;
        }

        setProcessingAdviserId(adviserId);
        router.post(
            studentRoutes.adviserRequests.store.url(),
            { adviser_id: adviserId },
            {
                preserveScroll: true,
                onFinish: () => setProcessingAdviserId(null),
            },
        );
    };

    return (
        <StudentLayout title="Invite Adviser" subtitle="Browse advisers and request mentorship for your group">
            <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-5">
                <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <Link href={studentRoutes.group.url()} className="font-medium text-slate-600 transition-colors hover:text-slate-900">
                        Capstone Group
                    </Link>
                    <ChevronRight className="h-3 w-3 text-slate-400" />
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-4 text-xs text-slate-600 shadow-sm">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="text-xs font-semibold text-slate-900">Group Context</p>
                            <p className="text-xs text-slate-500">
                                {group
                                    ? `${group.name}${group.programSet ? ` • ${group.programSet}` : ''}${
                                          group.academicYear ? ` • ${group.academicYear}` : ''
                                      }`
                                    : 'No group detected for your account yet.'}
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                            {currentAdviser ? (
                                <span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700">
                                    Adviser Assigned: {currentAdviser.name}
                                </span>
                            ) : (
                            ...
                            {hasPendingRequest ? (
                                <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-700">
                                    Pending Request: {pendingRequest?.adviser_name ?? 'Selected adviser'}
                                </span>
                            ) : null}
                        </div>
                    </div>
                                    ...
                                    <div className="flex-1 p-5">
                                        ...
                                        <div className="mt-4 flex gap-2">
                                            <button
                                                ...
                                                disabled={isDisabled}
                                                className={`inline-flex flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-[11px] font-semibold shadow-sm transition ${
                                                    isDisabled
...
export default AdviserSelection;
