    ...
    const rows: PastEvalRow[] = useMemo(
        () => [
            ...
            },
            {
                id: 'p2',
                group: 'Group Omega',
                projectTitle: 'Barangay Service Portal',
                defenseType: 'Outline',
                defenseDate: '2026-02-14',
                score: 76,
                verdict: 'Re-Defense',
            },
            {
                id: 'p3',
                group: 'Group Sigma',
                projectTitle: 'Inventory Forecasting Tool',
                defenseType: 'Pre-Deployment',
                defenseDate: '2026-01-30',
                score: 84,
                verdict: 'Approved',
            },
        ],
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((r) => (
                                    <tr key={r.id} className="hover:bg-slate-50">
                                        <td className="py-3 font-medium whitespace-nowrap text-slate-900">{r.group}</td>
                                        <td className="min-w-[220px] py-3 text-slate-700">{r.projectTitle}</td>
                                        <td className="py-3 whitespace-nowrap text-slate-700">{r.defenseType}</td>
                                        <td className="py-3 whitespace-nowrap text-slate-700">{r.defenseDate}</td>
                                        <td className="py-3 text-slate-700 tabular-nums">{r.score}</td>
                                        <td className="py-3">
                                            <span
                                                className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${verdictPill(r.verdict)}`}
                                            >
                                                {r.verdict}
                                            </span>
                                        </td>
