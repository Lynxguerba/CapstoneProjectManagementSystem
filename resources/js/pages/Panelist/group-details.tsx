...
const PanelistGroupDetails = () => {
    ...
    const group = {
        ...
        description: 'A capstone system that streamlines attendance recording and reporting using QR scanning and analytics dashboards.',
    ...
    const evaluationHistory = useMemo(
        () => [
            { id: 'h1', defenseType: 'Concept', date: '2026-02-10', score: 84, verdict: 'Approved', notes: 'Scope aligned; proceed to outline.' },
            {
                ...
                verdict: 'Pending',
                notes: 'Awaiting your evaluation submission.',
            },
        ],
        [],
    );
            ...
            <div className="space-y-6">
                ...
                >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <div className="text-xs font-semibold tracking-wide text-slate-500 uppercase">{group.name}</div>
                            <div className="mt-1 text-2xl font-bold text-slate-900">{group.projectTitle}</div>
                            <div className="mt-3 max-w-3xl text-sm text-slate-600">{group.description}</div>
                        ...
                        <div className="w-full rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-5 lg:w-80">
                ...
                {activeTab === 'overview' ? (
                    <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 space-y-1">
                                        {group.coPanelists.map((p) => (
                                            <div
                                                key={p}
                                                className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 font-semibold text-slate-800"
                                            >
                                                {p}
                                            </div>
                                        ))}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-200 text-slate-600">
                                        ...
                                        <th className="py-3 text-left font-semibold">Verdict</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {evaluationHistory.map((h) => (
                                        <tr key={h.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="max-w-xl py-3 text-slate-700">{h.notes}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
