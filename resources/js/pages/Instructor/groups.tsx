            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-6 overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-slate-200 text-slate-600">
                                    ...
                                    <th className="py-4 text-left font-semibold">Members</th>
                                    <th className="py-4 text-left font-semibold">Section</th>
                                    <th className="py-4 text-left font-semibold">Adviser</th>
                                    <th className="py-4 text-left font-semibold">Concept</th>
                                    <th className="py-4 text-left font-semibold">Defense</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((g) => (
                                    <tr key={g.id} className="transition-colors hover:bg-slate-50">
                                        ...
                                        <td className="py-4">
                                            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(g.conceptStatus)}`}>
                                                {g.conceptStatus}
                                            </span>
                                        </td>
                                        <td className="py-4">
                                            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(g.defenseStatus)}`}>
                                                {g.defenseStatus}
                                            </span>
                                        </td>
                                        <td className="py-4">
                                            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(g.paymentStatus)}`}>
                                                {g.paymentStatus}
                                            </span>
                                        </td>
                                        <td className="py-4">
                        ...
                        {filtered.length === 0 ? <div className="py-10 text-center text-sm text-slate-500">No groups match your filters.</div> : null}
                    </div>
                </div>
            </motion.div>
        </InstructorLayout>
