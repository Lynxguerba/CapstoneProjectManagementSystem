        ...
        <InstructorLayout title="Verdict Management" subtitle="Finalize outcomes based on panel results (UI only)">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5">
                                ...
                                <div className="mt-2">
                                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(current.recommendation)}`}>
                                        {current.recommendation}
                                    </span>
                                </div>
                            </div>
                            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5">
                                <div className="text-sm font-semibold text-slate-500">Suggested Verdict</div>
                                <div className="mt-2">
                                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(current.suggestedVerdict)}`}>
                                        {current.suggestedVerdict}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="mt-4 flex flex-wrap gap-3">
                                <button
                                    ...
                                    className="rounded-xl bg-gradient-to-r from-teal-600 to-teal-700 px-6 py-3 font-semibold text-white transition-all hover:shadow-lg"
                                >
                                    Approve
                                </button>
                                <button
                                    onClick={() => setVerdict('Re-Defense')}
                                    className="rounded-xl bg-gradient-to-r from-rose-600 to-rose-700 px-6 py-3 font-semibold text-white transition-all hover:shadow-lg"
                                >
                                    Re-Defense
                                </button>
                                <button
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-200 text-slate-600">
                                        ...
                                        <th className="py-4 text-left font-semibold">Average</th>
                                        <th className="py-4 text-left font-semibold">Recommendation</th>
                                        <th className="py-4 text-left font-semibold">Final Verdict</th>
                                        <th className="py-4 text-left font-semibold">Updated</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {rows.map((r) => (
                                        <tr key={r.id} className="transition-colors hover:bg-slate-50">
                                            ...
                                            <td className="py-4">
                                                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(r.recommendation)}`}>
                                                    {r.recommendation}
                                                </span>
                                            </td>
                                            <td className="py-4">
                                                <span
                                                    className={`rounded-full px-3 py-1 text-xs font-semibold ${badge(r.finalVerdict ?? 'Pending')}`}
                                                >
                                                    {r.finalVerdict ?? 'Pending'}
                                                </span>
                                            </td>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p className="mt-1 text-sm text-slate-500">Latest actions</p>

                    <div className="mt-6 space-y-3">
                        {history.map((h, idx) => (
                            <div key={idx} className="rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
