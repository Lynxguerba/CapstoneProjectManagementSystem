...
const ConceptsPage = () => {
    ...
    const tabStyle = (isActive: boolean) =>
        isActive ? 'bg-slate-900 text-white shadow' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50';
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex flex-wrap gap-2">
                            <button
                                className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${tabStyle(tab === 'Pending')}`}
                                onClick={() => setTab('Pending')}
                            >
                                Pending
                            </button>
                            <button
                                className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${tabStyle(tab === 'Approved')}`}
                                onClick={() => setTab('Approved')}
                            >
                                Approved
                            </button>
                            <button
                                className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${tabStyle(tab === 'Rejected')}`}
                    ...
                    <div className="mt-6 overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-slate-200 text-slate-600">
                                    ...
                                    <th className="py-4 text-left font-semibold">Last Updated</th>
                                    <th className="py-4 text-left font-semibold">Attachment</th>
                        ...
                        {filtered.length === 0 ? <div className="py-10 text-center text-sm text-slate-500">No concepts in this category.</div> : null}
                    </div>
                </div>
            </motion.div>
        </InstructorLayout>
