...
const ManageUserActionModal = ({ open, user, onClose, onSave }: ManageUserActionModalProps) => {
    const [editableUser, setEditableUser] = React.useState<ManagedUser | null>(user);
                ...
                <div className="space-y-4 p-4">
                    <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
                        ...
                        <p className="text-xs text-emerald-800">
                            {editableUser.name} ({editableUser.email})
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(event) => setEditableUser({ ...editableUser, name: event.target.value })}
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(event) => setEditableUser({ ...editableUser, email: event.target.value })}
                            className="mt-1.5 w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select
                                ...
                                onChange={(event) => setEditableUser({ ...editableUser, role: event.target.value as UserRole })}
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(event) => setEditableUser({ ...editableUser, status: event.target.value as UserStatus })}
                                className="mt-1.5 w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm capitalize focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500"
                            >
                    ...
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                        Created at: {editableUser.createdAt}
                    </div>
                </div>
