...
type BulkEnrollForm = {
    ...
    rows: Array<{ student_id: number }>;
};

type BulkEnrollStudentsModalProps = {
    open: boolean;
    onClose: () => void;
    programSetId: number;
...
const BulkEnrollStudentsModal = ({
    ...
    const enrolledEmailSet = React.useMemo(() => {
        return new Set(
            enrolledStudents
    ...
    const studentDirectory = React.useMemo(() => {
        const directory = new Map<string, StudentOption>();

        availableStudents.forEach((student) => {
            const email = student.email?.trim().toLowerCase();
            if (email) {
                directory.set(email, student);
            }
        });

        enrolledStudents.forEach((student) => {
            const email = student.email?.trim().toLowerCase();
            if (!email || directory.has(email)) {
                return;
            }
    ...
    useEffect(() => {
        if (open) {
            setData('program_set_id', programSetId);
        }
    }, [open, programSetId, setData]);

    useEffect(() => {
        const selectedRows = previewRows
            .filter((row) => row.issues.length === 0 && row.student && selectedRowLinesSet.has(row.line))
            .map((row) => ({
    ...
    const parseFile = async (file: File) => {
        ...
        const requiredHeaders = ['last_name', 'first_name', 'email'] as const;
        ...
        const normalizedProgramSet = normalizeProgram(programSetProgram);
        ...
        const parsedPreviewRows: PreviewRow[] = lines.slice(1).map((line, lineIndex) => {
            ...
            if (matchedStudent && normalizedProgramSet && studentProgram !== normalizedProgramSet) {
                issues.push('Program does not match this section.');
            }
    ...
    const importRows = () => {
        ...
        post('/instructor/students/bulk-enroll', {
            preserveScroll: true,
            preserveState: false,
            onSuccess: () => {
                clearUploadState();
                ...
                >
                    <div className="flex items-center justify-between border-b border-emerald-200 bg-gradient-to-r from-emerald-50 to-emerald-100 px-4 py-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-bold text-emerald-900">Bulk Enroll Students</h2>
                        </div>
                        <button
                            type="button"
                            onClick={closeAll}
                    ...
                    <div className="space-y-4 p-4">
                        ...
                        <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
                            ...
                            <span className="text-xs text-slate-500">Click to browse and preview before enrolling.</span>
                    ...
                    >
                        <div className="flex items-center justify-between border-b border-emerald-200 bg-gradient-to-r from-emerald-50 to-emerald-100 px-4 py-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold text-emerald-900">Review CSV Enrollment</h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="space-y-4 p-4">
                                            ...
                                            <tr key={`${row.line}-${row.email}`} className={row.issues.length > 0 ? 'bg-rose-50' : ''}>
                                                ...
                                                <td className="px-3 py-2">
                                                    <div className="text-sm font-medium text-slate-800">
                                                        {row.student ? resolveStudentName(row.student) : `${row.last_name}, ${row.first_name}`}
                                                    </div>
                                                </td>
                                                <td className="px-3 py-2 text-slate-600">{row.email}</td>
                                                <td className="px-3 py-2 text-slate-600">{row.program ?? '-'}</td>
                                                <td className="px-3 py-2">
                                                    {row.issues.length > 0 ? (
                                                        ...
                                                        <span className="text-xs text-emerald-700">Ready to enroll</span>
                                                    )}
                                                </td>
                                                <td className="px-3 py-2">
                                                    {row.issues.length > 0 ? (
                            ...
                            {Object.keys(errors).length > 0 ? (
                                <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">
                                    Enrollment failed. Please fix row issues and try again.
                                </div>
                            ) : null}
...
export default BulkEnrollStudentsModal;
