...
const AdviserGroupsModal = ({
    ...
    const [summary, setSummary] = React.useState<AdviserGroupsSummary | null>(null);
    ...
    const loadGroups = React.useCallback(
        async (signal?: AbortSignal) => {
            ...
            try {
                const url = adviserAssignment.groups.url(
                    { adviser: adviserId },
                ...
                if (!response.ok) {
                    throw new Error('Failed to load adviser groups.');
                }
                ...
                setErrorMessage('Unable to load adviser groups right now.');
            } finally {
                if (!signal?.aborted) {
                    setIsLoading(false);
                }
    ...
    }, [open, adviserId, academicYear, loadGroups]);
                ...
                <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
                    ...
                    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                        ...
                        {!isLoading && groups.length === 0 ? (
                            <div className="py-10 text-center text-sm text-slate-500">No groups assigned for this adviser.</div>
                        ) : null}
                    </div>
                </div>
            </motion.div>
...
export default AdviserGroupsModal;
