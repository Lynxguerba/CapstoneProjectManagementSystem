...
type StatusErrorPageProps = {
    ...
    primaryAction?: 'back' | 'reload' | 'login';
};
...
const goLogin = (): void => {
    router.visit('/login', { preserveScroll: false, replace: true });
};

export default function StatusErrorPage({ code, title, message, primaryActionLabel = 'Go Back', primaryAction = 'back' }: StatusErrorPageProps) {
    const primaryActionHandler = primaryAction === 'reload' ? reloadPage : primaryAction === 'login' ? goLogin : goBack;
    const PrimaryIcon = primaryAction === 'reload' ? RefreshCcw : ArrowLeft;
        ...
        <>
            <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-green-100 px-4 sm:px-6 lg:px-8">
                <div className="pointer-events-none absolute inset-0">
                    <div className="absolute top-[-8rem] left-[-5rem] h-72 w-72 rounded-full bg-emerald-300/20 blur-3xl" />
                    <div className="absolute right-[-6rem] bottom-[-10rem] h-96 w-96 rounded-full bg-green-400/15 blur-3xl" />
                </div>
                ...
                <div className="relative z-10 w-full max-w-2xl rounded-3xl border border-emerald-100/80 bg-white/80 p-8 shadow-[0_24px_80px_rgba(21,128,61,0.16)] backdrop-blur-sm sm:p-12">
                    ...
                    <div className="mb-8 text-center">
                        ...
                        <p className="mx-auto max-w-md text-base leading-relaxed text-slate-600 sm:text-lg">{message}</p>
                    </div>
                        ...
                        >
                            ...
                            <PrimaryIcon
                                className={`relative z-10 h-5 w-5 transition-transform duration-200 ${
                                    primaryAction === 'reload' ? 'group-hover:rotate-180' : 'group-hover:-translate-x-1'
                                }`}
                            />
